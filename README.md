...
The extension creates no dedicated tables. Persisted Agreement and Proponent tags use `extensions.kv_entry`, with `owner_type` and `owner_id`. Agreement tags follow the Agreement’s Agency; Proponent tags follow the acting user’s active agency roles. Shared KV ownership is registered by the host, not redeclared here.
